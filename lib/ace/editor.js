...
var Editor =function(renderer, doc) {
    ...
    this.renderer.addEventListener("gutterclick", this.$onGutterClick);
...
(function(){
    ...
    this.setDocument = function(doc) {
        ...
        if (this.doc) {
            ...
            this.doc.removeEventListener("changeCodeFolding", this.$onDocumentChangeCodeFolding);
        ...
        this.$onDocumentChangeCodeFolding = this.onDocumentChangeCodeFolding.bind(this);
        this.doc.addEventListener("changeCodeFolding", this.$onDocumentChangeCodeFolding);
    ...
    this.detectCodeFoldingAreas = function(){
        if (!!this.$codeFoldingPending) {
            return;
        }
        
        // perform code folding async to not block the browser during navigation
        var self = this;
        this.$codeFoldingPending = true;
        setTimeout(function() {
            self.$codeFoldingPending = false;
            self.doc.detectCodeFoldingAreas();
        }, 10);
    };
    ...
    this.onDocumentChangeCodeFolding = function() {
        this.renderer.setCodeFoldingAreas(this.doc.getCodeFoldingAreas());
    };
    
    this.onGutterClick = function(e){
        this.doc.toggleCodeFoldingArea(e.row);
    };
