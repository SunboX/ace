    ...
    this.setCodeFoldingAreas = function(areas) {
        ...
        for(var i = 0; i < areas.length; i++){
            if(typeof areas[i] != 'undefined'){
                if(areas[i].open){
                    this.$codeFoldingLines[areas[i].start - 1] = 'ace_folding-open';
                    for(var j = areas[i].start + 1; j < areas[i].end; j++){
                          this.$codeFoldingLines[j - 1] = 'ace_folding-vertical-line';
                    }
                    this.$codeFoldingLines[areas[i].end - 1] = 'ace_folding-end';
                } else {
                    this.$codeFoldingLines[areas[i].start - 1] = 'ace_folding-closed';
                }
            }
        }
    };
